...
function AdminVoteRow({ vote, onDeleted }: { vote: AdminVote; onDeleted: () => Promise<unknown> }) {
  ...
  return <div className={styles.adminVoteRow}><span>{title}<small>{new Date(vote.startsAt).toLocaleString()}</small><small>Winner +{vote.winnerReward} · loser +{vote.loserReward} VOX</small></span><em>{vote.status}</em><strong>{vote.participantCount} votes</strong><button className={styles.deleteVoteButton} title="Delete vote" onClick={() => setConfirming(true)}><Trash2 size={16} />Delete</button>{confirming && createPortal(<div className={styles.adminDialogBackdrop} onMouseDown={(event) => { if (event.currentTarget === event.target && !deleting) setConfirming(false); }}><section className={styles.adminDialog} role="dialog" aria-modal="true" aria-labelledby={`delete-vote-${vote.id}`}><span className={styles.adminDialogIcon}><AlertTriangle /></span><p className={styles.eyebrow}>Permanent admin action</p><h2 id={`delete-vote-${vote.id}`}>Delete this vote?</h2><p><strong>{title}</strong> will disappear from MyVoice. Existing votes, results, and the VOX ledger will remain stored for integrity.</p>{error && <span className={styles.errorText} role="alert">{error}</span>}<div className={styles.adminDialogActions}><button className={styles.secondary} disabled={deleting} onClick={() => setConfirming(false)}>Keep vote</button><button className={styles.confirmDeleteButton} disabled={deleting} onClick={() => void remove()}>{deleting ? 'Deleting…' : 'Delete vote'}</button></div></section></div>, document.body)}</div>;
}
...
function VoteComposer({ onSaved }: { onSaved: () => void }) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const vote = await api<{ id: string }>('/admin/votes', { method: 'POST', body: JSON.stringify({ startsAt: new Date(String(data.get('startsAt'))).toISOString(), endsAt: new Date(String(data.get('endsAt'))).toISOString(), winnerReward: Number(data.get('winnerReward')), loserReward: Number(data.get('loserReward')), translations: [{ language: 'en', title: data.get('titleEn'), description: data.get('descriptionEn') }, { language: 'ru', title: data.get('titleRu'), description: data.get('descriptionRu') }], options: [{ position: 1, translations: [{ language: 'en', text: data.get('oneEn') }, { language: 'ru', text: data.get('oneRu') }] }, { position: 2, translations: [{ language: 'en', text: data.get('twoEn') }, { language: 'ru', text: data.get('twoRu') }] }] }) }, true);
      await api(`/admin/votes/${vote.id}/schedule`, { method: 'POST' }, true); setMessage('Vote scheduled'); setOpen(false); onSaved();
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); }
  };
  return <section className={styles.composer}><button className={styles.primary} onClick={() => setOpen(!open)}>Create vote</button>{message && <span>{message}</span>}{open && <form className={styles.form} onSubmit={save}><div className={styles.twoCols}><label>Starts (local)<input type="datetime-local" name="startsAt" required /></label><label>Ends (local)<input type="datetime-local" name="endsAt" required /></label></div><div className={styles.rewardSettings}><div><small>Outcome rewards</small><strong>Additional VOX after the result</strong></div><div className={styles.twoCols}><label>Winner reward<input type="number" name="winnerReward" min="0" step="1" defaultValue="0" required /></label><label>Loser reward<input type="number" name="loserReward" min="0" step="1" defaultValue="0" required /></label></div><p>Every participant still receives the base +10 VOX. A tie has no additional outcome reward.</p></div><label>English title<input name="titleEn" required minLength={3} maxLength={240} /></label><label>English description<textarea name="descriptionEn" required minLength={10} maxLength={3000} /></label><div className={styles.twoCols}><label>Option 1<input name="oneEn" required maxLength={160} /></label><label>Option 2<input name="twoEn" required maxLength={160} /></label></div><label>Русский заголовок<input name="titleRu" required minLength={3} maxLength={240} /></label><label>Русское описание<textarea name="descriptionRu" required minLength={10} maxLength={3000} /></label><div className={styles.twoCols}><label>Вариант 1<input name="oneRu" required maxLength={160} /></label><label>Вариант 2<input name="twoRu" required maxLength={160} /></label></div><button className={styles.primary}>Create and schedule</button></form>}</section>;
}
